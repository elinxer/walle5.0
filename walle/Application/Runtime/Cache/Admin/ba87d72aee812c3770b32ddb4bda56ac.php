...
<div class="row-fluid sortable">
	<div class="box span12">
		...
		<?php if($user["name"] == 'admin'): ?><a href="<?php echo U('User/add_user');?>" class="btn">新增用户</a>
		<p>&nbsp;</p><?php endif; ?>
			<table class="table table-striped table-bordered bootstrap-datatable datatable">
			  ...
			  <tbody>
			  <?php if(is_array($member)): foreach($member as $key=>$member): ?><tr>
					<td><?php echo ($member["name"]); ?></td>
					<td class="center"><?php echo (change_time($member["reg_time"],$member.reg_time)); ?></td>
					<td class="center">
					<?php if($member['name'] == C('RBAC_SUPERADMIN')): ?>超级管理员<?php else: ?>
					<?php if(is_array($member['role'])): foreach($member['role'] as $key=>$role): echo ($role["name"]); ?>(<?php echo ($role["title"]); ?>)<br><?php endforeach; endif; endif; ?>
					</td>
					<td class="center">
						<span class="label label-success">
						<?php if($member['status']): ?>启用<?php else: ?>禁用<?php endif; ?>
						</span>
					</td>
					<td class="center">
						...
						<a class="btn btn-danger" href="<?php echo U('User/delete_user', array('id'=>$member['id']));?>" onclick="return confirm('删除？');">
