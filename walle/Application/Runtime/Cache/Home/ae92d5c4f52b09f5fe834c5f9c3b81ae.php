...
<div>
<form action="<?php echo U('Shoujiao/add' ,array('type'=>$type));?>" method="post">
	<table class="bordered">
	    <thead>
	    <tr>   
	        <th>&nbsp;<?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?>名称</th>
	        <th>&nbsp;<?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?>金额</th>
	        <th>&nbsp;<?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?>时间</th>
	        <th>&nbsp;<?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?>说明（15&nbsp;字以内）</th>
	    </tr>
	    </thead>
	    <tbody><tr>   
	        <td style="width:26%;">
	        <div class="input-group input-group-lg">
	        	<span class="input-group-addon"><?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?></span>
	...
	<div style="text-align:right;">
		...
		<input type="hidden" name="type"  <?php if($type): ?>value="<?php echo ($type); ?>"<?php else: ?>value="0"<?php endif; ?> />
...
<div>
&nbsp;<?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?>总额:<?php echo ($sum); ?> &nbsp;&nbsp;&nbsp;&nbsp;
当前<?php if($type): ?>收缴<?php else: ?>分配<?php endif; ?>：<?php if($now > 250): ?><span class="label label-danger"><?php echo ($now); ?></span><?php else: ?><span class="label label-success"><?php echo ($now); ?></span><?php endif; ?>
<!-- 分页 -->
...
<div>
	<table class="zebra">
	    ...
	    <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
	        ...
	        <a href="<?php echo U('Shoujiao/delete',array('id'=>$vo['id'],'belong'=>'add','type'=>$type));?>" class="btn btn-default" onclick="return confirm('确定要删除吗')">删除</a>
	        <a href="<?php echo U('Shoujiao/update',array('belong'=>'add','id'=>$vo['id'],'type'=>$type));?>" class="btn btn-warning">修改</a>
	        &nbsp;&nbsp;
	        </td>
