...
class ZhichuController extends HomeController {
    public function index(){
   		// 信息输出
   		$this->sum = M('bill_data')->where(array('uid' => $_SESSION['uid'],'type_id'=>1))->sum('value');
    	$this->page('bill_data' , 15 , array('type_id'=> 1 ,'uid'=>$_SESSION['uid']));
    ...
    public function recorde(){
    	if (IS_POST) {
    		...
    		$this->sum = M('bill_data')->where(array('uid' => $_SESSION['uid'],'type_id'=>1))->sum('value');
    		$this->now = M('bill_data')->where(array('uid' => $_SESSION['uid'],'type_id'=>1,'create_time'=>array('gt',$nowtime)))->sum('value');
    		$this->page('bill_data' , 5 , array('type_id'=> 1 ,'uid'=>$_SESSION['uid']));
    ...
    public function update(){
    	if (IS_POST) {
			...
			$this->url = 'Zhichu/'.$_GET['belong']; //返回链接
