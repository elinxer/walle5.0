...
<div>
	<table class="zebra">
	    ...
	    <?php if(is_array($list)): foreach($list as $key=>$vo): if($vo['is_lock']): ?><tr class="alert alert-danger">
	        <td><a href="<?php echo U('Savemoney/timeline' ,array('id'=>$vo['id']));?>" class="btn btn-info" >View</a></td>
	        <td><?php echo ($vo["name"]); ?>(未实现)</td>
	        <td><?php echo ($vo["value"]); ?></td>
	        <td><?php echo (change_time($vo["time"],5)); ?></td>
	        <td><?php echo ($vo["content"]); ?></td>
	        <td style="text-align:right;">
	        <a href="<?php echo U('Savemoney/delete',array('id'=>$vo['id']));?>" class="btn btn-default" onclick="return confirm('目标未实现，确定删除？')">删除</a>
