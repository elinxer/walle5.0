...
<div>
	<table class="table table-bordered" style="width:60%;">
		...
		<td colspan="4" style="text-align:right;">
			<a href="<?php echo U('Billtype/add_bill_type');?>" class="btn btn-large btn-primary">添加类型&nbsp;</a>
		</td>
		</tr></thead>
		<tbody>
		<tr>
		<td>类型ID</td><td>类型名称</td><td>类型英文名称</td><td>操作</td>
		</tr>
		<?php if(is_array($bill_type)): foreach($bill_type as $key=>$bill): ?><tr>
			<td><?php echo ($bill["type_id"]); ?></td><td><?php echo ($bill["title"]); ?></td><td><?php echo ($bill["name"]); ?></td>
			<td><?php if($user['name'] == 'admin'): ?><a href="<?php echo U('Billtype/delete_type' ,array('id'=>$bill['id']));?>" onclick="return confirm('删除？');">&nbsp;删除</a><?php else: ?>删除<?php endif; ?></td>
