...
<div>
	<table class="table">
		<thead><tr><td><a href="<?php echo U('Nav/add_nav');?>" class="btn btn-large btn-primary">新增功能</a></td><td></td></tr></thead>
		<tbody>
		<tr>
		<td>功能名称</td><td>链接地址</td><td>打开方式</td><td>排序</td><td>上层ID</td><td>操作</td>
		</tr>
		<?php if(is_array($nav)): foreach($nav as $key=>$nav): ?><tr>
			<td><?php if($nav['type'] == 0): echo ($nav["title"]); else: ?>--<?php echo ($nav["title"]); endif; ?></td>
			<td><?php echo ($nav["url"]); ?></td>
			<td><?php echo ($nav["target"]); ?></td>
			<td><?php echo ($nav["sort"]); ?></td>
			<td><?php echo ($nav["type"]); ?></td>
			<td><a href="<?php echo U('Nav/edit_nav' , array('nav_id'=>$nav['id']));?>">编辑 </a>
			<?php if($user['name'] == 'admin'): ?><a href="<?php echo U('Nav/delete_nav' , array('nav_id'=>$nav['id']));?>" onclick="return confirm('删除？');">删除</a>
			<?php else: ?>删除<?php endif; ?>
			</td>
